...
export const ProfileDrawer: React.FC<ProfileDrawerProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "skills" | "projects" | "leadership"
  >("overview");
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                {profile.currentTitle} • Permanent Profile
              </p>
            </div>
          </div>
        ...
        <div className="mx-6 mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-start space-x-3">
          ...
          <div className="text-xs text-amber-800">
            <span className="font-semibold block text-amber-900">
              Truthful Application Policy
            </span>
            All resume bullet points and cover letters generated from this
            profile adhere to 100% truthful claims. No fabricated experience,
            unearned certifications, or fake metrics.
          </div>
        </div>

        {/* Navigation Tabs */}
        <div className="px-6 border-b border-slate-200 mt-4 flex space-x-6 text-xs font-medium">
          {(["overview", "skills", "projects", "leadership"] as const).map(
            (tab) => (
              <button
                key={tab}
            ...
            <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {profile.openTo.map((item, i) => (
                        <span
                          key={i}
                          className="bg-slate-200 text-slate-800 px-2 py-0.5 rounded text-[10px]"
                        >
                          {item}
                        </span>
                      ))}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <h3 className="font-semibold text-slate-800 uppercase tracking-wider text-[11px]">
                    Professional Summary
                  </h3>
                  {!isEditing ? (
                    <button
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {skillList.map((skill, i) => (
                      <span
                        key={i}
                        className="bg-white text-slate-700 border border-slate-200 shadow-sm px-2.5 py-1 rounded-md text-[11px]"
                      >
                        {skill}
                      </span>
                    ))}
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1 py-1">
                    {proj.tech.map((t, i) => (
                      <span
                        key={i}
                        className="text-[10px] bg-indigo-50 text-indigo-700 border border-indigo-200 px-2 py-0.5 rounded"
                      >
                        {t}
                      </span>
                    ))}
